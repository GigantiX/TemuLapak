...
class EditMerchantProfileViewModel extends _$EditMerchantProfileViewModel {
  ...
  /// Handles all logic for validating and saving the merchant's profile.
  Future<void> saveChanges({
    ...
    try {
      ...
      Logger.log("VM - Starting merchant profile update validation");

      if (merchantCategory.isEmpty) {
        throw Exception("Silakan pilih minimal satu kategori");
      }

      final List<Product> products = productFields
          .map((field) => Product(
  ...
  void navigateToMerchantProfile(BuildContext context) {
...
class EditPickImageViewModel extends _$EditPickImageViewModel {
  ...
  Future<void> pickImage(ImageSource source) async {
    try {
      state = AppState.loading();
      Logger.log(
          "Opening ${source == ImageSource.camera ? 'camera' : 'gallery'} to pick image for edit");
